...
class OnboardingTooltips extends _$OnboardingTooltips {
  ...
  OnboardingTooltipsData build() {
    ...
    return OnboardingTooltipsData(
      ...
      tooltipControllers: {
        for (var e in OnboardingTooltip.values) e: SuperTooltipController()
      },
  ...
  Set<OnboardingTooltip> _initialise() {
    final tooltips =
        ref.read(userServiceProvider).userData.onboardingTooltipsShown;

    ref.listen(
        userServiceProvider.select((e) => e.userData.onboardingTooltipsShown),
        (_, tooltips) {

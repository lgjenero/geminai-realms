...
import 'package:geminai_realms/services/location/ip_location_service.dart';
...
class UserService extends _$UserService {
  ...
  void initialize() async {
    ...
    if (!user.countryCode.isValidCountryCode) {
      final location =
          await ref.read(ipLocationServiceProvider).getIpLocation();
  ...
  Future<String?> login(String email, String password) async {
    ...
    final result =
        await AuthService(auth: FirebaseAuth.instance).login(email, password);
    if (result == null) {
      await loadUserDataFromFirestore();
    }
    state = state.copyWith(loaded: true);
  ...
  Future<String?> signUp(String email, String password) async {
    state = state.copyWith(loaded: false);
    final result =
        await AuthService(auth: FirebaseAuth.instance).signUp(email, password);
    if (result == null) {
      await loadUserDataFromFirestore();
    }
    state = state.copyWith(loaded: true);
  ...
  Future<void> loadUserDataFromFirestore() async {
    ...
    if (userId != null) {
      final user = await FirestoreService(firestore: FirebaseFirestore.instance)
          .getUserData();
      ...
      if (user != null) {
        merged = merged.copyWith(
          countryCode: user.countryCode.isValidCountryCode
              ? user.countryCode
              : userData.countryCode,
          flag: user.flag,
          finishedLevel: user.finishedLevel > userData.finishedLevel
              ? user.finishedLevel
              : userData.finishedLevel,
          achievements: user.achievements.length > userData.achievements.length
              ? user.achievements
              : userData.achievements,
          introShown: user.introShown || userData.introShown,
          onboardingTooltipsShown: user.onboardingTooltipsShown.length >
                  userData.onboardingTooltipsShown.length
  ...
  void addAchievement(String achievement) {
    final userData = state.userData
        .copyWith(achievements: [...state.userData.achievements, achievement]);
    state = state.copyWith(userData: userData);
